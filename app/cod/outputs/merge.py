...
from .utils import cols_meta, cwd, get_all_cols, get_attrs, get_pop_cols, logging
...
def get_df():
    df1 = pd.read_parquet(outputs / "../meta-fb/adm4_population.parquet")
...
def add_fraction(df):
    ...
    for l in range(4, -1, -1):
        ...
        df1["pop_src"] = "cod"
        df2 = pd.read_parquet(outputs / f"../meta-fb/adm{l}_population.parquet")
        df2 = df2[get_ids(l) + ["t"]]
    ...
    return (
        pd.concat(frames)
        .sort_values(by="adm4_id")
        .drop(columns=["pop_lvl"])
        .drop_duplicates()
    )
...
def export_df(df):
    for l in range(4, -1, -1):
        df1 = (
            df.groupby(get_ids(l) + cols_meta, dropna=False)
            .sum(numeric_only=True, min_count=1)
            .reset_index()
        )
